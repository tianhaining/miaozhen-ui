...
const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => t(`mz.datepicker.weeks.${ week }`)),
    dayNames: weeks.map(week => t(`mz.datepicker.weeks.${ week }`)),
    monthNamesShort: months.map(month => t(`mz.datepicker.months.${ month }`)),
    monthNames: months.map((month, index) => t(`mz.datepicker.month${ index + 1 }`)),
    amPm: ['am', 'pm']
  };
};
