<template>
  <transition name="mz-zoom-in-top" @after-leave="$emit('dodestroy')">
    <div
      v-show="visible"
      class="mz-picker-panel mz-date-range-picker mz-popper"
      :class="[{
        'has-sidebar': $slots.sidebar || shortcuts,
        'has-time': showTime
      }, popperClass]">
      <div class="mz-picker-panel__body-wrapper">
        <slot name="sidebar" class="mz-picker-panel__sidebar"></slot>
        <div class="mz-picker-panel__sidebar" v-if="shortcuts">
          <button
            type="button"
            class="mz-picker-panel__shortcut"
            v-for="shortcut in shortcuts"
            @click="handleShortcutClick(shortcut)">{{shortcut.text}}</button>
        </div>
        <div class="mz-picker-panel__body">
          <div class="mz-date-range-picker__time-header" v-if="showTime">
            <span class="mz-date-range-picker__editors-wrap">
              <span class="mz-date-range-picker__time-picker-wrap">
                <mz-input
                  size="small"
                  :disabled="rangeState.selecting"
                  ref="minInput"
                  :placeholder="t('mz.datepicker.startDate')"
                  class="mz-date-range-picker__editor"
              ...
              <span class="mz-date-range-picker__time-picker-wrap">
                <mz-input
                  size="small"
                  :disabled="rangeState.selecting"
                  :placeholder="t('mz.datepicker.startTime')"
                  class="mz-date-range-picker__editor"
            ...
            <span class="mz-icon-arrow-right"></span>
            <span class="mz-date-range-picker__editors-wrap is-right">
              <span class="mz-date-range-picker__time-picker-wrap">
                <mz-input
                  size="small"
                  :disabled="rangeState.selecting"
                  :placeholder="t('mz.datepicker.endDate')"
                  class="mz-date-range-picker__editor"
              ...
              <span class="mz-date-range-picker__time-picker-wrap">
                <mz-input
                  ...
                  :disabled="rangeState.selecting"
                  ref="maxInput"
                  :placeholder="t('mz.datepicker.endTime')"
                  class="mz-date-range-picker__editor"
          ...
          <div class="mz-picker-panel__content mz-date-range-picker__content is-left">
            <div class="mz-date-range-picker__header">
              <button
                type="button"
                @click="leftPrevYear"
                class="mz-picker-panel__icon-btn mz-icon-d-arrow-left"></button>
              <button
                type="button"
                @click="leftPrevMonth"
                class="mz-picker-panel__icon-btn mz-icon-arrow-left"></button>
              <button
                type="button"
                @click="leftNextYear"
                v-if="unlinkPanels"
                ...
                class="mz-picker-panel__icon-btn mz-icon-d-arrow-right"></button>
              <button
                type="button"
                @click="leftNextMonth"
                v-if="unlinkPanels"
                ...
                class="mz-picker-panel__icon-btn mz-icon-arrow-right"></button>
              <div>{{ leftLabel }}</div>
            </div>
            <date-table
          ...
          <div class="mz-picker-panel__content mz-date-range-picker__content is-right">
            <div class="mz-date-range-picker__header">
              <button
                type="button"
                @click="rightPrevYear"
      ...
      <div class="mz-picker-panel__footer" v-if="showTime">
        <mz-button
          size="mini"
          type="text"
          class="mz-picker-panel__link-btn"
          @click="handleClear">
          {{ t('mz.datepicker.clear') }}
        </mz-button>
        <mz-button
          plain
          size="mini"
...
<script type="text/babel">
  ...
  export default {
    ...
    computed: {
      ...
      leftLabel() {
        return this.leftDate.getFullYear() + ' ' + this.t('mz.datepicker.year') + ' ' + this.t(`mz.datepicker.month${ this.leftDate.getMonth() + 1 }`);
      },

      rightLabel() {
        return this.rightDate.getFullYear() + ' ' + this.t('mz.datepicker.year') + ' ' + this.t(`mz.datepicker.month${ this.rightDate.getMonth() + 1 }`);
      },
    ...
    components: { TimePicker, DateTable, MzInput, MzButton }
  };
</script>
