<template>
  <transition name="mz-zoom-in-top" @after-enter="handleEnter" @after-leave="handleLeave">
    <div
      v-show="visible"
      class="mz-picker-panel mz-date-picker mz-popper"
      :class="[{
      ...
      <div class="mz-picker-panel__body-wrapper">
        ...
        <div class="mz-picker-panel__body">
          <div class="mz-date-picker__time-header" v-if="showTime">
            <span class="mz-date-picker__editor-wrap">
              <mz-input
                :placeholder="t('mz.datepicker.selectDate')"
                :value="visibleDate"
                size="small"
            ...
            <span class="mz-date-picker__editor-wrap">
              <mz-input
                ...
                :placeholder="t('mz.datepicker.selectTime')"
                :value="visibleTime"
                size="small"
          ...
          <div
            class="mz-date-picker__header"
            :class="{ 'mz-date-picker__header--bordered': currentView === 'year' || currentView === 'month' }"
            v-show="currentView !== 'time'">
            <button
              type="button"
              @click="prevYear"
              :aria-label="t(`mz.datepicker.prevYear`)"
              class="mz-picker-panel__icon-btn mz-date-picker__prev-btn mz-icon-d-arrow-left">
            </button>
            <button
              type="button"
              @click="prevMonth"
              v-show="currentView === 'date'"
              :aria-label="t(`mz.datepicker.prevMonth`)"
              class="mz-picker-panel__icon-btn mz-date-picker__prev-btn mz-icon-arrow-left">
            </button>
            <span
              @click="showYearPicker"
              role="button"
              class="mz-date-picker__header-label">{{ yearLabel }}</span>
            <span
              @click="showMonthPicker"
              v-show="currentView === 'date'"
              role="button"
              class="mz-date-picker__header-label"
              :class="{ active: currentView === 'month' }">{{t(`mz.datepicker.month${ month + 1 }`)}}</span>
            <button
              type="button"
              @click="nextYear"
              :aria-label="t(`mz.datepicker.nextYear`)"
              class="mz-picker-panel__icon-btn mz-date-picker__next-btn mz-icon-d-arrow-right">
            </button>
            <button
              type="button"
              @click="nextMonth"
              v-show="currentView === 'date'"
              :aria-label="t(`mz.datepicker.nextMonth`)"
              class="mz-picker-panel__icon-btn mz-date-picker__next-btn mz-icon-arrow-right">
            </button>
          </div>

          <div class="mz-picker-panel__content">
            <date-table
              v-show="currentView === 'date'"
              @pick="handleDatePick"
...
<script type="text/babel">
  ...
  export default {
    ...
    components: {
      TimePicker, YearTable, MonthTable, DateTable, MzInput, MzButton
    },
    ...
    computed: {
      ...
      yearLabel() {
        const yearTranslation = this.t('mz.datepicker.year');
        if (this.currentView === 'year') {
