<template>
  ...
  >
    <div class="mz-submenu__title" ref="submenu-title" :style="paddingStyle">
      <slot name="title"></slot>
      <i :class="{
        'mz-submenu__icon-arrow': true,
        'mz-icon-arrow-down': rootMenu.mode === 'vertical',
        'mz-icon-caret-bottom': rootMenu.mode === 'horizontal'
      }">
      </i>
    </div>
    <template v-if="rootMenu.mode === 'horizontal'">
      <transition name="mz-zoom-in-top">
        <ul class="mz-menu" v-show="opened"><slot></slot></ul>
      </transition>
    </template>
